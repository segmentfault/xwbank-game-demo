...
(function (EasyGame) {
    ...
    var StreamReader = (function () {
        function StreamReader(data, length) {
            this.buffer = data;
            this.pos = 0;
        ...
        StreamReader.prototype.checkBound = function (size) {
            if (this.pos + size > this.dataView.byteLength)
                throw new Error("out of stream limit.");
        };
        StreamReader.prototype.readInt = function () {
            var v;
            this.checkBound(1);
        ...
        StreamReader.prototype.read = function (t) {
            var v;
            var len;
            if (typeof (t) === "number") {
                switch (t) {
        ...
        StreamReader.prototype.readBits = function (count) {
            var bits = [];
            var b = 0;
        ...
        StreamReader.prototype.readArray = function (t) {
        ...
        StreamReader.prototype.readDict = function (k, t) {
            var len = this.readInt();
            if (len < 0)
                return null;
            var dict = {};
        ...
        StreamReader.prototype.readTuple = function (t) {
            var tuple = [];
    ...
    var StreamWriter = (function () {
        ...
        StreamWriter.prototype.w = function () {
            var args = [];
        ...
        StreamWriter.prototype.checkBound = function (size) {
    ...
    var ProxyBase = (function () {
        function ProxyBase(conn) {
            this.conn = conn;
            this.onError = function (e) { alert("Error: " + e.message); };
        }
        ProxyBase.prototype.send = function (stream, onResult) {
            var buf = new Uint8Array(stream.byteBuffer.buffer, 0, stream.pos);
            this.conn.send(buf, onResult);
        };
        ProxyBase.prototype.getResult = function (t, resolve, reject) {
            return function (s) {
                var r = s.readInt();
                if (r) {
    ...
    var NotifyListener = (function () {
        function NotifyListener(url, session, networkEvent) {
        ...
        NotifyListener.prototype.addStub = function (stub) {
            this.stubs[stub.pid] = stub;
        };
        NotifyListener.prototype.stop = function () {
            this.xhr.abort();
            this.xhr = null;
        };
        NotifyListener.prototype.run = function () {
            var _this = this;
            var ctx = this;
            var xhr = this.xhr;
            ...
            xhr.onload = function () {
                if (xhr.status === 200) {
                    ...
                    _this.networkEvent(NetworkEvent.Error, xhr.statusText);
                }
            };
    ...
    var RequestConnection = (function () {
        function RequestConnection(url, session, multiplex, networkEvent) {
            this.xhr = new XMLHttpRequest();
            this.reqId = 1;
            this.nextStep = [];
            this.retry = 0;
        ...
        Object.defineProperty(RequestConnection.prototype, "queueLength", {
            get: function () {
        ...
        RequestConnection.prototype.send = function (data, onResponse, resend) {
            var _this = this;
            var ctx = this;
            if (ctx.buzy) {
                ctx.nextStep.push(function () { ctx.send(data, onResponse); });
                ...
                xhr.onload = function () {
                    ...
                    if (xhr.status === 200) {
                        ...
                        _this.networkEvent(NetworkEvent.Error, xhr.status);
                        return;
                    }
                    if (ctx.nextStep.length > 0) {
                        ...
                        _this.networkEvent(NetworkEvent.Recover);
                };
                xhr.ontimeout = function () {
                    ctx.buzy = false;
                    ctx.retry++;
    ...
    var WebSocketConnectionAdapter = (function () {
        function WebSocketConnectionAdapter(path, conn) {
            this.path = path;
            this.conn = conn;
        }
        WebSocketConnectionAdapter.prototype.send = function (data, onResponse) {
            this.conn.send(this.path, data, onResponse);
        };
        Object.defineProperty(WebSocketConnectionAdapter.prototype, "queueLength", {
            get: function () {
    ...
    var StubBase = (function () {
        function StubBase(pid) {
            this._reqCB = {};
            this._reqId = 0;
            this.pid = pid;
        }
        StubBase.prototype.newReq = function (onResp) {
            this._reqId++;
            this._reqCB[this._reqId] = onResp;
            return this._reqId;
        };
        StubBase.prototype.asyncCB = function (s) {
            var id = s.readInt();
            var cb = this._reqCB[id];
            delete this._reqCB[id];
            cb(s);
        };
        StubBase.prototype.dispatch = function (s) {
            if (this.dispMethods) {
