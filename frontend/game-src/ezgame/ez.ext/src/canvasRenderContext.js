        ...
        var RC = {
            ...
            fillRect: function (w, h, transform) {
                ...
                if (typeof (_fill) !== "string") {
                    ...
                    if (PROFILING && profile) {
                        profile.transfromChange++;
                        profile.fillRect++;
                        profile.commands.push(`fillrect gradient`);
                    }
                    ...
                    if (PROFILING && profile) {
                        profile.fillRect++;
                        profile.commands.push(`fillrect ${_fill}`);
                    }
                    _ctx.fillRect(0, 0, w, h);
                }
                else {
            ...
            drawImage: function (tex, transform, w, h, srcRect, quad) {
                ...
                if (setTransform(t)) {
                    if (PROFILING && profile) {
                        profile.drawImage++;
                        profile.commands.push(`drawimage ${tex.name}`);
                    }
                    if (srcRect) {
                        if (m)
            ...
            drawImageRepeat: function (tex, transform, width, height, repeat) {
                ...
                if (PROFILING && profile) {
                    profile.drawImage++;
                    profile.commands.push(`drawImageRepeat ${tex.name}`);
                }
                setTransform(t, true);
            ...
            drawImageS9: function (texture, transform, s9, width, height, srcRect) {
                ...
                if (PROFILING && profile) {
                    profile.drawImage += 9;
                    profile.commands.push(`drawImageS9 ${texture.name}`);
                }
            ...
            setAlphaBlend: function (alpha, blendMode) {
                if (_blendMode != blendMode) {
                    ...
                    _ctx.globalCompositeOperation = CompositeOperation[_blendMode];
                    if (PROFILING && profile)
                        profile.commands.push(`setCompositeOperation ${blendMode}`);
                }
                if (_alpha != alpha) {
                    _alpha = alpha;
                    _ctx.globalAlpha = _alpha;
                    if (PROFILING && profile)
                        profile.commands.push(`setAlpha ${_alpha}`);
                }
            },
            drawText: function (content, transform, x, y, w, h, a, stroke) {
                ...
                if (_font != content.font) {
                    ...
                    if (PROFILING && profile)
                        profile.commands.push(`setFont ${content.font}`);
                }
                ...
                for (var i = 0; i < content.lines.length; i++) {
                    ...
                    if (stroke) {
                        ...
                        if (PROFILING && profile) {
                            profile.drawText++;
                            profile.commands.push(`strokeText ${l.text}`);
                        }
                    }
                    _ctx.fillText(l.text, (x0 + x) | 0, y | 0, w | 0);
                    if (PROFILING && profile) {
                        profile.drawText++;
                        profile.commands.push(`fillText ${l.text}`);
                    }
                    y += s;
                }
            },
            ...
            setFillColor: function (col) {
                if (_fill !== col) {
                    ...
                    _fill = col;
                    if (PROFILING && profile)
                        profile.commands.push(`setFillColor ${col}`);
                }
            },
            ...
            setFillGradient: function (g) {
                ...
                _ctx.fillStyle = grad;
                if (PROFILING && profile)
                    profile.commands.push(`setFillGradient`);
            }
        };
        return RC;
    };
