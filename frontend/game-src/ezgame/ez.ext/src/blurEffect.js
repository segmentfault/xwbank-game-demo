...
void main(){
    ...
    function getShader(horn, radius) {
        var name = `blur_${horn ? 'h' : 'v'}${radius}`;
        if (ez.Shader.has(name))
            return ez.Shader.get(name);
        var define = "";
        if (horn)
        ...
        var gl = ez.getGL();
        ez.Shader.register(name, new ez.Shader(ez.Shader.DefVS2D, define + fs, ["pos", "quad"], { texture0: gl.uniform1i, quads: gl.uniform4fv, PixelSize: gl.uniform1f, BlendWeights: gl.uniform1fv }));
        return ez.Shader.get(name);
    }
    class BlurStageSprite extends ez.SubStageSprite {
        ...
        set radius(val) {
            ...
            this._hornShader = getShader(true, val);
            this._vertShader = getShader(false, val);
    ...
    (function (ui) {
        class BlurGroup extends ui.Container {
            constructor(parent) {
                ...
                this._initProperties(BlurGroup.Properties);
