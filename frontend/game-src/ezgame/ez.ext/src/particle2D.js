...
void main(){
    ...
    function getShader(frames) {
        ...
        if (frames) {
            ...
            ez.Shader.register(name, new ez.Shader(`#define MAX_QUAD ${MaxFramesQuads}\n` + framesVS, ez.Shader.DefFS2D, ["pos", "quad"], { texture0: gl.uniform1i, quads: gl.uniform4fv, t: gl.uniform1fv }));
            return ez.Shader.get(name);
        }
        else {
            var name = "particle2d";
            if (ez.Shader.has(name))
                return ez.Shader.get(name);
            ez.Shader.register(name, new ez.Shader(`#define MAX_QUAD ${MaxFramesQuads}\n` + singleVS, ez.Shader.DefFS2D, ["pos", "quad"], { texture0: gl.uniform1i, quads: gl.uniform4fv, t: gl.uniform1fv }));
            return ez.Shader.get(name);
        }
    }
