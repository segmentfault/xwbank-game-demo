...
(function (inspector_1) {
    ...
    var Inspector = (function () {
        ...
        Inspector.prototype.setBound = function (b) {
        ...
        Inspector.prototype.getFPS = function () {
            connected = true;
            function s() {
                if (!connected)
                    return;
        ...
        Inspector.prototype.beginPickElement = function () {
        ...
        Inspector.prototype.getTextStyles = function () {
            sendMsgToDevtools("onTextStyles", ez.ui.getTextStyleNames());
        };
        Inspector.prototype.getEffectNames = function () {
            sendMsgToDevtools("onEffectNames", Object.getOwnPropertyNames(ez.Shader["lib"]));
        };
        Inspector.prototype.getImageResources = function () {
            sendMsgToDevtools("onImageResources", ez.allRes().filter(function (v) { return v.type == 4 || v.type == 5 || v.type == 13; }).map(function (v) { return v.name; }));
        };
        Inspector.prototype.getElementData = function (idx) {
            var _a;
        ...
        Inspector.prototype.setElementProp = function (idx, name, val) {
            console.log("setElementProp", idx, name, val);
            var e = this.elements[idx];
            e[name] = val;
        };
        Inspector.prototype.getUITree = function () {
        ...
        Inspector.prototype.showBorder = function (show) {
            console.log("showBorder");
            this.border.style.visibility = show ? "visible" : "hidden";
        };
        Inspector.prototype.setElementVisible = function (idx, v) {
            var e = this.elements[idx];
            if (!e)
                return;
            e.visible = v;
        };
        Inspector.prototype.recordDetailProfile = function () {
            ez.Profile.beginEventProfiling();
            ez.Profile.beginFrameProfiling(true, function (data) {
                sendMsgToDevtools("onFrameProfileData", data);
            });
        };
        Inspector.prototype.stodRecordProfile = function () {
            var events = ez.Profile.endEventProfiling();
            ez.Profile.endFrameProfiling();
            sendMsgToDevtools("onEventProfileData", events);
        };
        Inspector.prototype.endProfile = function () {
            ez.Profile.endFrameProfiling();
        };
        Inspector.prototype.beginProfile = function () {
            var minData;
            var maxData;
            var avgData;
            var count = 0;
            var lastUpdate = Date.now();
            ez.Profile.beginFrameProfiling(false, function (data) {
                if (count == 0) {
                    minData = {};
                    maxData = {};
                    avgData = {};
        ...
        Inspector.prototype.getTextureStats = function () {
    ...
    function installAsDevtools() {
        ...
        sendMsgToDevtools = function (fn) {
            var args = [];
            for (var _i = 1; _i < arguments.length; _i++) {
                args[_i - 1] = arguments[_i];
            }
            if (args.length > 0)
                window.postMessage({ source: 'ezgame-inspector', body: __spreadArrays([fn], args) }, "*");
            else
    ...
    function installWithWebServer() {
        ...
        sendMsgToDevtools = function (fn) {
            ...
            if (args.length > 0)
                ws.send(JSON.stringify(__spreadArrays([fn], args)));
            else
    ...
    function addTestcase(testcases) {
        for (var k in testcases) {
            ...
            if (!t.Arguments) {
                ...
                t.Arguments = args.map(function (arg) {
                    var t = arg.split("=");
                    var desc = { name: t[0] };
                    if (t.length > 1)
                        desc.default = t[1];
